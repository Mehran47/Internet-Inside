...
# Network Setup:
def setup_network():
    # Delete previous POSTROUTING as much as possible
    subprocess.run("iptables -t nat -D POSTROUTING -o eth0 -j MASQUERADE", shell=True)
    subprocess.run("iptables -t nat -D POSTROUTING -o eth0 -j MASQUERADE", shell=True)
    subprocess.run("iptables -t nat -D POSTROUTING -o eth0 -j MASQUERADE", shell=True)
    ...
    subprocess.run("sudo ip route add 192.168.1.1 dev tun1", shell=True)

    # Routing process
    subprocess.run("sudo iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE", shell=True)

    # Forwarding rules
    subprocess.run("sudo iptables -A FORWARD -i eth0 -o tun1 -m state --state RELATED,ESTABLISHED -j ACCEPT", shell=True)
    subprocess.run("sudo iptables -A FORWARD -i tun1 -o eth0 -j ACCEPT", shell=True)


    # Create new TUN interfaces
    subprocess.run("sudo ip tuntap add mode tun dev tun1", shell=True)
    # subprocess.run("sudo ip tuntap add mode tun dev tun2", shell=True)
    # time.sleep(1)

    # Assign IP addresses to the TUN interfaces
    subprocess.run("sudo ip addr add 192.168.1.1/24 dev tun1", shell=True)
    # subprocess.run("sudo ip addr add 192.168.2.2/24 dev tun2", shell=True)
    # time.sleep(1)

    # Bring up the TUN interfaces
    subprocess.run("sudo ip link set dev tun1 up", shell=True)

    # Display the status of TUN interfaces
    subprocess.run("sudo ip addr show", shell=True)
    subprocess.run("iptables -t nat -L -v -n", shell=True)
...
# initilize the tun:
def tun_initialization(Tun_name = 'tun1'):
...
if __name__ == "__main__":
    ...
    setup_network()
    tun = tun_initialization("tun1")
   
    parser = argparse.ArgumentParser(description='NRF24L01+ test')
    parser.add_argument('--src', dest='src', type=str, default='ttttt', help='NRF24L01+\'s source address')
    parser.add_argument('--dst', dest='dst', type=str, default='rrrrr', help='NRF24L01+\'s destination address')
...
##############################################################
    ...
    for nrf in [rx_nrf, tx_nrf]:
        ...
        nrf.spi_speed = 8000000  # Adjust SPI speed if necessary
    #call teh tun function:

    rx_process = Process(target=rx, args=(rx_nrf, tun, args.rxchannel, bytes(args.dst, 'utf-8'), args.cnt))
    tx_process = Process(target=tx, args=(tx_nrf, args.txchannel, bytes(args.src, 'utf-8'), args.cnt, tun))
    rx_process.start()
    tx_process.start()
    time.sleep(1)
    rx_process.join()
